...
Favourites = ('[COLOR %s]reorder_favourites > [/COLOR]' % TEXT_GENERAL)
...
CHANGELOG_TEXT = '[CR][CR][CR]C H A N G E L O G [LIGHT] (newest at the top)[/LIGHT][CR][CR]Version code x.y.z attributes[CR]x = major change / y = number of user interface buttons / z = number of menu items relating to the interface[CR][CR]version 1.2.4 (2 user interface buttons & 4 menu items relating to the interface)[CR]- menu updated with User Information dialogue box (Instructions / Notes / Development / Changelog)[CR]- menu updated with Developer, Name, Version and Addon ID[CR]- user interface ids in xml renumbered[CR]- user interface remote scrolling within borders[CR]- user interface images and layout improved[CR]- variables and functions reworked[CR]- dialogue boxes and logs reworked[CR]- simplified addon.xml content to reduce maintenance[CR][CR]version 1.0.0 (2 user interface buttons & 4 menu items relating to the interface)[CR]- code from Order Favourites 1.2.3a by doko-desuka[CR]- user interface resized to full screen[CR]- improved layout using new images and default image[CR]- visible scrollbar and resized text[CR]- menu and dialogue boxes reworked[CR]- user instructions added to addon.xml[CR]- icon.png changed and fanart.jpg added'
...
if '/User_Interface' in PLUGIN_URL:
	...
	User_Interface = xbmcgui.ListItem('[B]%s[/B][COLOR %s]  >[/COLOR]' % (Addon_Title, TEXT_GENERAL))
